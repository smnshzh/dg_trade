...
def main():
    ...
    start_id = st.number_input("Enter Start Product ID", min_value=100, value=100)
    end_id = st.number_input("Enter End Product ID", min_value=start_id, value=6850999)
    if st.button("Start Processing"):
        progress_bar = st.progress(0)
        status_text = st.empty()

        total_products = end_id - start_id + 1
        for i, product_id in enumerate(range(start_id, end_id + 1)):
